...
// 由四个命名子配置组成，支持 TOML 反序列化。
type AppConfig struct {
	Server         ServerConfig         `toml:"server"`
	RateLimit      RateLimitConfig      `toml:"rateLimit"`
	Access         AccessConfig         `toml:"access"`
	TokenWhiteList TokenWhiteListConfig `toml:"tokenWhiteList"`
}
...
// TokenWhiteListConfig Token 白名单配置。
type TokenWhiteListConfig struct {
...
func DefaultConfig() *AppConfig {
	return &AppConfig{
		...
		TokenWhiteList: TokenWhiteListConfig{
...
//	GLOBAL_RATE          - 全局限速（字节/秒），0=不限速
//	IP_REQUEST_LIMIT     - IP 请求限流（次/小时）

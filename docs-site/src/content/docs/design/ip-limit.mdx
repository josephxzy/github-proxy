...
请求进入
  ├─ IP 限流（次数/小时）    ← 新增，本次实现
  ├─ Token 白名单（带宽免限） ← 已有
  └─ 带宽限速（per-user + global） ← 已有
```

| 层 | 维度 | 粒度 | 白名单豁免 |
|:---|:---|:---|:---|
| IP 限流 | 请求次数 | 每小时每 IP | ✅ Token 在白名单 → 跳过 |
| 带宽限速 | 下载速度 | per-user + global | ✅ Token 在白名单 → 跳过 |
...
| `handlers/ip_limiter_unix.go` | Unix 平台的 `setRLimit`（可选，复用现有模式） |
| `handlers/ip_limiter_windows.go` | — |

> 实际上不需要平台特定文件。IP 限流是纯 Go 逻辑，不涉及系统调用。

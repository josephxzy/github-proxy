...
|:---|:---|:---|:---|
| Release Asset | 206, Content-Range: bytes 0-0/13377735 | ✅ | 有 |
| Raw 文件 | 206, Content-Range: bytes 0-0/7712 | ✅ | 有 |
| Archive (codeload) | 200, 无 Content-Range | ❌ | 无 |
| Archive (github.com) | 302 → codeload, 无 | ❌ | 无 |
| Blob 页面 | 无 Content-Range | ❌ | 无 |

> 测试脚本：在代理服务器上运行 [`size_probe_test.py`](https://github.com/josephxzy/github-proxy/blob/main/docs-site/src/assets/size_probe_test.py) 可自行验证。支持自定义 URL 和 Token。
...
`handlers/response_writer.go` 负责写入客户端。每 64KB flush，数据立即可达。配合水位线反压和双层限速（详见限速设计文档）。
